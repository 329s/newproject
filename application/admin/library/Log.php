...
class Log
{
    ...
    {
        $path = $this->dir . $file;
        if (strpos($file, '.') === 0 || false !== strpos($file, '..') || is_dir($path)) return false;
        return $path;
    }
    ...
    public function filter(&$row, $item)
    {
        ...
        if (isset($filter['level']) && $filter['level'] != strtolower($item['level'])) return false;
        if (isset($filter['method']) && $filter['method'] != strtolower($item['method'])) return false;
        if (isset($filter['url']) && !strpos($item['url'], $filter['url'])) return false;
        if (isset($filter['time'])) {
            ...
            if ($timeArr[0] > $itemTimeInt || $timeArr[1] < $itemTimeInt) return false;
        }
        $row[] = $item;
        return true;
    }
    ...
    {
        return $this->setSelected($this->directory($this->dir));
    }
